<div class="wrapper">
    <nav id="sidebar">
        <div class="sidebar-header">
            <h3 class="text-center">
                <a class="text-decoration-none" href="index.php?act=null">
                    <span class="font-weight-bold">Trang quản trị</span>
                </a>
            </h3>
        </div>
        <ul class="list-unstyled components">
            <li class="dropdown">
                <a href="index.php?act=statistics" class="dashboard">
                    <i class="fa-solid fa-chart-simple"></i><span>Quản lý thống kê</span></a>
            </li>
            <li class="dropdown">
                <a href="index.php?act=category">
                    <i class="fa-solid fa-table-list"></i><span>Quản lý danh mục</span>
                </a>
            </li>
            <li class="dropdown">
                <a href="index.php?act=product">
                    <i class="fa-solid fa-cart-shopping"></i><span>Quản lý sản phẩm</span>
                </a>
            </li>
            <li class="dropdown">
                <a href="index.php?act=comment">
                    <i class="fa-regular fa-comment-dots"></i></i><span>Quản lý bình luận</span>
                </a>
            </li>
            <li class="dropdown">
                <a href="index.php?act=order">
                    <i class="fa-solid fa-bag-shopping"></i><span>Quản lý đơn hàng</span>
                </a>
            </li>
            <li class="dropdown">
                <a href="index.php?act=account">
                    <i class="fa-solid fa-users"></i><span>Quản lý người dùng</span>
                </a>
            </li>
            <li>
                <a href="../index.php">
                    <i class="fa-solid fa-arrow-right-from-bracket"></i><span>Trang người dùng</span>
                </a>
            </li>
        </ul>
    </nav>